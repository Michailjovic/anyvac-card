...
| `scale_y` | number | same as `scale` | Manual seating only. Independent Y-axis scale, applied to the robot's own map on its own axes BEFORE `rotation` turns it into place. Undefined means isotropic (identical to `scale`, i.e. today's behavior) — set it only to correct a real aspect distortion in the robot's own map that a single `scale` can't reach (e.g. a precisely-measured external floorplan still doesn't quite match the robot's raw map proportions). These are the robot's own local axes, not the screen's — at 90°/270° rotation `scale`/`scale_y` end up affecting the floorplan's vertical/horizontal extent respectively, not the other way round, since rotation is a separate later step. The **editor** hides this: its two sliders are always labelled/behave as "horizontal"/"vertical" relative to what's currently on screen, swapping which of `scale`/`scale_y` each one reads and writes based on the current `rotation` (`isRot90`, seatfit.ts) — only someone editing this YAML directly needs to think in local axes at all. |
| `offset_x` / `offset_y` | number | `0` | Manual seating only. In the screen's own frame (the floorplan's), unlike `scale`/`scale_y` above — `rotation` never swaps which one is "horizontal". |
...
**Editor "Swap ↔/↕" toggle (Maps tab, 2026-09-17 field report):** the card can auto-rotate
its whole map area 90° to fit a narrow card box (`layout` config below), independently of
any vacuum's own `rotation`. Home Assistant's card-config dialog renders its live preview in
a box of whatever width *that dialog* happens to use, which is not always the width the same
card renders at on your real dashboard — so the dialog's preview and your dashboard can
legitimately pick different auto-rotations and disagree on which way is "horizontal". The
editor cannot detect your dashboard's real width from inside that dialog, so instead every
↔/↕-labelled slider in the Maps tab (Scale, Offset, Image offset, Room position/size) is
driven by a manual, editor-only "Swap ↔/↕" toggle near the top of the tab (on top of each
field's own `rotation`-based swap, where it has one) — turn it on if a slider marked ↔
visibly moves something ↕ *on your real dashboard*. This toggle is UI-only: it is never saved
to your YAML, and resets the next time you open the editor.
...
| `scale` | number | `1` | |
| `offset_x` / `offset_y` | number | `0` | In the screen's own frame — `rotation` never swaps which one is "horizontal" (editor: **"Image offset ↔/↕"**, subject to the Maps tab's "Swap ↔/↕" toggle, see the per-vacuum `map` table above). |
...
| `map_x` / `map_y` | number (%) | live from integration | Always | **Omit both** to let the room's position track the integration's live geometry automatically (renames/re-shapes in the Roborock app show up with no manual re-import). **Set both** for a fixed position — required in degraded mode, and doubles as a fit anchor for auto-seating in merged mode. Already in the shared floorplan's own (screen) frame — editor: **"X ↔/↕"**, subject to the Maps tab's "Swap ↔/↕" toggle (see `map.scale_y` above). |
| `map_w` / `map_h` | number (%) | — | Always | Setting either activates rectangle mode (a drawn box instead of a point marker). Drag the box or its corner handles in the editor preview. Same frame/toggle as `map_x`/`map_y` above (editor: **"Width ↔/↕"**). |
